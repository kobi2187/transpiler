## XLang Type Definitions
##
## This module defines the core XLang AST node types used as an intermediate
## representation for multi-language transpilation to Nim.
##
## XLang serves as a language-agnostic AST that captures common programming
## constructs from multiple source languages (Python, Go, C#, Java, TypeScript, etc.)
...
type
  ...
  XLangNodeKind* = enum
    ...
    xnkMixinDecl, xnkTemplateDecl, xnkMacroDecl
    xnkInclude, xnkExtend
    ...
    xnkStringInterpolation, xnkCastExpr, xnkAssignExpr
    # Language-specific expressions
    xnkInstanceVar, xnkClassVar, xnkGlobalVar, xnkProcLiteral, xnkProcPointer
    xnkArrayLit, xnkNumberLit
    ...
    xnkDynamicType, xnkAbstractType, xnkFunctionType
    ...
    # Nim-specific (for Nim as input language)
  ...
  XLangNode* = ref object
    ## Core XLang AST node - a variant object that can represent any language construct
    ...
    # New node kinds for language-specific features
    of xnkModuleDecl:
    ...
    of xnkCastExpr:
      ...
      castType*: XLangNode
    of xnkAssignExpr:
      assignTarget*: XLangNode
      assignValue*: XLangNode
...
proc parseVersion*(v: string): XLangVersion =
  ## Parse a version string like "1.0.0" into a version tuple
...
proc isVersionCompatible*(ast_version, supported_version: XLangVersion): bool =
  ## Check if an AST version is compatible with supported version
  ## Major version must match exactly
  ## Minor version of AST cannot be greater than supported
