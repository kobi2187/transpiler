...
import passes/resource_to_try_finally
import passes/switch_expr_to_case
import passes/lock_to_with_lock
...
proc buildNimPassRegistry*(): Table[TransformPassID, TransformPass] =
  ...
  reg[tpExtensionMethods] = newTransformPass(tpExtensionMethods, toClosure(transformExtensionMethods), @[])  # operates on common kinds, no tracking
