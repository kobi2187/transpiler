...
proc buildNimPassRegistry*(): Table[TransformPassID, TransformPass] =
  ...
  reg[tpAsyncNormalization] = newTransformPass(tpAsyncNormalization, toClosure(transformAsyncNormalization), @[], @[xnkAwaitExpr, xnkYieldExpr])
  ...
  reg[tpPythonGenerators] = newTransformPass(tpPythonGenerators, toClosure(transformPythonGenerator), @[], @[xnkYieldStmt, xnkFuncDecl]) 
