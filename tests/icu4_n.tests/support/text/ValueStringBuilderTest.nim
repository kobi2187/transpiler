...
type
  ValueStringBuilderTest = ref object
...
proc Ctor_Span_CanAppend*() =
    var vsb = ValueStringBuilder(seq[char])
Assert.AreEqual(0, vsb.Length)
vsb.Append('a')
Assert.AreEqual(1, vsb.Length)
Assert.AreEqual("a", vsb.ToString)
proc Ctor_InitialCapacity_CanAppend*() =
    var vsb = ValueStringBuilder(1)
Assert.AreEqual(0, vsb.Length)
vsb.Append('a')
Assert.AreEqual(1, vsb.Length)
Assert.AreEqual("a", vsb.ToString)
...
proc Append_Char_CapacityExceeded_TracksMaxLength*() =
    var output: Span<char> = newSeq[char](5)
    var sb = ValueStringBuilder(output)
...
Assert.AreEqual(sb.Length, vsb.Length)
Assert.AreEqual(sb.ToString, vsb.ToString)
proc Append_String_BeyondCapacity_SetsCapacityExceeded*() =
    var sb = ValueStringBuilder(newSeq[char](5))
Assert.IsFalse(sb.CapacityExceeded)
sb.Append("012345678")
Assert.IsTrue(sb.CapacityExceeded)
proc Append_String_BeyondCapacity_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](5))
Assert.AreEqual(0, sb.Length)
...
Assert.AreEqual(sb.Length, vsb.Length)
Assert.AreEqual(sb.ToString, vsb.ToString)
proc Append_CharInt_CapacityExeeded_SetsMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](5))
      var i: int = 1
      while i <= 100:
sb.Append(cast[char](i), i)
...
Assert.AreEqual(5, sb.Length)
Assert.AreEqual(5050, sb.MaxLength)
proc Append_PtrInt_MatchesStringBuilder*() =
    var sb = StringBuilder
    var vsb = ValueStringBuilder
      var i: int = 1
      while i <= 100:
...
Assert.AreEqual(sb.Length, vsb.Length)
Assert.AreEqual(sb.ToString, vsb.ToString)
proc AppendSpan_CapacityExceeded_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](128))
      var i: int = 1
      while i <= 1000:
          var s: string = i.ToString
          var span: Span<char> = sb.AppendSpan(s.Length)
s.AsSpan.CopyTo(span)
++i
...
Assert.AreEqual(sb.Length, vsb.Length)
Assert.AreEqual(sb.ToString, vsb.ToString)
proc Insert_IntCharInt_TracksMaxLength*() =
    var sb = ValueStringBuilder
    var rand = Random(42)
      var i: int = 1
      while i <= 100:
          var index: int = rand.Next(sb.Length)
sb.Insert(index, cast[char](i), i)
++i
Assert.AreEqual(5050, sb.Length)
Assert.AreEqual(5050, sb.MaxLength)
    sb.Length = 0
...
Assert.AreEqual(sb.Length, vsb.Length)
proc ToString_ClearsBuilder_ThenReusable*() =
    var Text1: string = "test"
    var vsb = ValueStringBuilder
vsb.Append(Text1)
Assert.AreEqual(Text1.Length, vsb.Length)
    var s: string = vsb.ToString
Assert.AreEqual(Text1, s)
Assert.AreEqual(0, vsb.Length)
Assert.AreEqual(string.Empty, vsb.ToString)
Assert.True(vsb.TryCopyTo(Span[char].Empty, _))
    var Text2: string = "another test"
vsb.Append(Text2)
...
Assert.AreEqual(0, vsb.Length)
proc TryCopyTo_ClearsBuilder_ThenReusable*() =
    var Text1: string = "test"
    var vsb = ValueStringBuilder
vsb.Append(Text1)
Assert.AreEqual(Text1.Length, vsb.Length)
    var dst: Span<char> = seq[char]
Assert.True(vsb.TryCopyTo(dst,     var charsWritten: int))
...
Assert.AreEqual(Text2.Length, vsb.Length)
Assert.AreEqual(Text2, vsb.ToString)
proc Dispose_ClearsBuilder_ThenReusable*() =
    var Text1: string = "test"
    var vsb = ValueStringBuilder
vsb.Append(Text1)
Assert.AreEqual(Text1.Length, vsb.Length)
...
proc EnsureCapacity_IfRequestedCapacityWins*() =
    var builder = ValueStringBuilder(newSeq[char](32))
builder.EnsureCapacity(65,     var newCapacity: int)
...
proc Remove_BeyondCapacity_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](5))
Assert.AreEqual(0, sb.Length)
Assert.AreEqual(0, sb.MaxLength)
sb.Append("012345678")
...
Assert.AreEqual(4, sb.Length)
Assert.AreEqual(13, sb.MaxLength)
proc AppendCodePoint_CapacityExceeded_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](7))
sb.Append("foo bar")
...
proc TestAppendCodePointBmp*() =
    var sb = ValueStringBuilder(newSeq[char](16))
sb.Append("foo bar")
    var codePoint: int = 97
sb.AppendCodePoint(codePoint)
...
proc InsertCodePoint_CapacityExceeded_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](7))
sb.Append("foo bar")
    var codePoint: int = 97
...
proc Test_Append_String*() =
    var sb: ValueStringBuilder = ValueStringBuilder(newSeq[char](16))
sb.Append("ab")
...
proc reverseTest(org: String, rev: String, back: String) =
    var sb: ValueStringBuilder = ValueStringBuilder(newSeq[char](32))
sb.Append(org)
sb.Reverse
    var reversed: String = sb.AsSpan.ToString
...
Assert.AreEqual(back, reversed)
    sb.Length = 0
sb.Append(org)
    var copy: String = sb.AsSpan.ToString
...
Assert.AreEqual(0, sb.Length)
    sb.Length = 0
sb.Append("abcde")
    var str: String = sb.AsSpan.ToString
sb.Delete(0, sb.Length - 0)
sb.Append("YY")
...
Assert.AreEqual("000011", sb.AsSpan.ToString)
Assert.AreEqual(6, sb.Length)
    var buffer: ValueStringBuilder = ValueStringBuilder(newSeq[char](8))
...
proc Test_IndexOf_String_CultureSensitivity*() =
    ...
    var sb: ValueStringBuilder = ValueStringBuilder(newSeq[char](fixture.Length))
sb.Append(fixture)
...
proc Test_IndexOf_String_CultureSensitivity_LargeString*() =
    ...
    var searchFor: string = "ሄኢቌጕኬ቏ቖኋዘዻ"
    var sb: ValueStringBuilder = ValueStringBuilder(fixture.Length)
sb.Append(fixture)
      let context = CultureContext("ru-MD")
<unhandled: nnkDefer>
...
proc Length_BeyondCapacity_TracksMaxLength*() =
    var sb = ValueStringBuilder(newSeq[char](5))
Assert.AreEqual(0, sb.Length)
Assert.AreEqual(0, sb.MaxLength)
sb.Append("012345678")
