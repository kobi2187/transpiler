...
proc TestTryFormatInt64_AgainstRbnfDecimalFormat_SpellOut*() =
    var number: long = 123456789012345678
    var destination: Span<char> = newSeq[char](CharStackBufferSize)
    for locale in NumberFormat.GetUCultures(UCultureTypes.AllCultures):
AssertTryFormatFor(number, locale, NumberPresentation.SpellOut, destination)
proc TestTryFormatInt64_AgainstRbnfDecimalFormat_SpellOut_ar_EH*() =
    var number: long = 123456789012345678
    var destination: Span<char> = newSeq[char](CharStackBufferSize)
AssertTryFormatFor(number, UCultureInfo("ar_EH"), NumberPresentation.SpellOut, destination)
proc AssertTryFormatFor(number: long, locale: UCultureInfo, presentation: NumberPresentation, destination: Span[char]) =
    var culture = locale.ToCultureInfo
    var rbnf = RuleBasedNumberFormat(locale, presentation)
...
proc Test_PluralFormatUnitTest*(culture: string, number: double, pluralPattern: string, pluralType: PluralType, decimalPattern: string, expected: string, assertMessage: string) =
